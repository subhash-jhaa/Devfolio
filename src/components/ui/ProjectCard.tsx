...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  const CardContent = (
    <div 
      ...
      className={cn(
        "relative flex w-full min-h-[180px] sm:min-h-[230px] flex-col rounded-2xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4 hover:border-gray-300 dark:hover:border-zinc-500 hover:shadow-xl shadow-sm",
        project.slug && "cursor-pointer"
      )}
    >
      <div className="flex items-center justify-between mb-3">
        {/* Logo and buttons handled separately if they have links */}
        <div
          className="w-6 h-6 text-xs bg-white dark:bg-zinc-700 overflow-hidden"
        >
      ...
      <div className="mt-auto">
        ...
        <div className="flex flex-wrap gap-1.5">
          {project.techStack.map((tech) => (
            <span key={tech} className="inline-flex items-center gap-1 text-[10px] font-medium rounded-md px-1.5 py-1 bg-gray-50 dark:bg-zinc-700 text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-600 shadow-sm hover:scale-110">
